...
import DevWarning from '../components/dev-warn';
...
export default {
  ...
  methods: {
    async fetch_live_data() {
      ...
      let korone_lives = [];
      const channel_id = 'UChAnqc_AY5_I3Px5dig3X1Q';
      fetched.live.forEach(function (item) {
        if (item.channel === channel_id) {
          item.type = 'live';
          korone_lives.push(item);
        }
      });
      ...
      korone_lives.forEach(function (item, index, object) {
